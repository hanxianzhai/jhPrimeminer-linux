	...
	ProbablePrimeChainTestFast(mpzChainOrigin, testParams);
      ...
      if(nProbableChainLength >= block->serverData.nBitsForShare)
      {
         ...
         *(uint8*)(blockRawData+writeIndex) = (uint8)lengthBN; // varInt (we assume it always has a size low enough for 1 byte)
         ...
         std::cout << sNow << " - "<< nPrimorialMultiplier <<" - SHARE FOUND! - DIFF:" << shareDiff << " - Th#:" << threadIndex << " TYPE:" << nCandidateType << std::endl;
         std::ofstream myfile;
         myfile.open ("found.txt",std::ios_base::out | std::ios_base::app);
         myfile << sNow << " - "<< nPrimorialMultiplier <<" - SHARE FOUND! - DIFF:" << shareDiff << " - Th#:" << threadIndex << " TYPE:" << nCandidateType << "\n";
         myfile.close();
